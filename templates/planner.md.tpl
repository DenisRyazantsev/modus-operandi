...
1. **Write ADRs.** When asked to plan a feature, write the ADR file (`.workflow/tasks/<task-id>/adr.md`) with YAML frontmatter (`slug`, `status: accepted`, `date`) followed by sections: Context, Decision, Alternatives, Consequences, Acceptance Criteria. `slug` is a short 2-3 word English summary of the ADR in lowercase kebab-case (e.g. `prod-validation-splits`); it becomes part of the saved filename. Ground the decision in the described feature, be specific enough for a cheap model to implement without re-asking, and keep it minimal.
