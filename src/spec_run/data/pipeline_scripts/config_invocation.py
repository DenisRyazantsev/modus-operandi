...
CONFIG_PATH = Path(
    ...
    or (Path(__file__).resolve().parent.parent.parent / "spec-run" / "config.yml")
)
...
def normalize_config(cfg: dict[str, Any]) -> dict[str, Any]:
    ...
    The installer does the same at install time (spec_run/config.py,
...
def build_specify_invocation(
    ...
    # ~/.config/spec-run/task-pipeline.yml) or a bare id in tests,
