...
#
# The backend is selected by SKLC_BACKEND (exported by the run-pipeline.py
...
if [ "$BACKEND" = "cursor" ]; then
  ...
  if [ -z "$ROLE_MODEL" ]; then
    echo "error: executor model is not set (SKLC_EXECUTOR_MODEL); run through run-pipeline.py or export it" >&2
    exit 2
  fi
