#!/usr/bin/env python3
"""spec-run - global launcher for the spec-run pipelines (console-script entry).

Installed by pip as the ``spec-run`` console script and callable from any
...
Usage:
  ...
  spec-run --backend cursor task "task description"   # override the backend for this run
  spec-run --help | -h

Examples:
  spec-run task "add a dark mode toggle"
  spec-run task "add a dark mode toggle" -i task_id=dark-mode
  spec-run --backend cursor task "add a dark mode toggle"
  spec-run review
  spec-run review --branch-diff
  spec-run edit
"""
...
from spec_run import bootstrap, paths, uninstall
from spec_run.edit_command import _run_edit
from spec_run.editor import resolve_editor
from spec_run.exceptions import EditRequested, HelpRequested, InvalidInvocation, UninstallRequested
...
# target (the spec_run.cli module).
__all__ = [
    "CONFIG",
    "REVIEW_WORKFLOW",
    "RUN_PIPELINE",
...
USAGE = """Usage: spec-run <subcommand> [args]
  ...
  spec-run uninstall [--yes]
      ...
      spec-run files under ~/.config/opencode), then run
      `pip uninstall spec-run` to remove the package itself. Asks for
