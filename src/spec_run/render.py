...
_DATA = files("spec_run").joinpath("data")
...
def render_agents(cfg: dict[str, Any], paths: Paths) -> None:
    ...
    (paths["agents"] / "planner.md").write_text(
        _agent_markdown(
            "Planner and reviewer for the spec-run task/review workflows",
            "0.3",
            planner["provider"],
            planner["model"],
            planner["reasoning"],
    ...
    (paths["agents"] / "executor.md").write_text(
        _agent_markdown(
            "Executor for the spec-run task/review workflows",
            "0.1",
            executor["provider"],
            executor["model"],
            executor["reasoning"],
...
# configured values are written into the workflow at install/`spec-run edit`
# time.
...
def _patch_workflow_numbers(data: dict[str, Any], cfg: dict[str, Any]) -> None:
    ...
    the validated config so `spec-run edit` re-applies a changed config
