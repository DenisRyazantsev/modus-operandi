...
def ensure_installed(layout: Paths) -> int:
    ...
    Reads ``install-version.txt`` (config base /spec-run/install-version.txt):
    ...
    try:
        ...
        return 1
    if was_installed:
        print(f"spec-run: updated to {__version__}", flush=True)
    else:
        print(f"spec-run: installed to {layout['config_dir']}", flush=True)
    return 0
