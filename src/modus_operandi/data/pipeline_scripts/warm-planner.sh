...
if [ -z "${MO_PLANNER_MODEL:-}" ] || [ -z "${MO_EXECUTOR_MODEL:-}" ]; then
  echo "error: cursor backend: planner/executor models are not set (MO_PLANNER_MODEL/MO_EXECUTOR_MODEL); run through run-pipeline.py or export them" >&2
  exit 2
fi
