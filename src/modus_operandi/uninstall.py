...
# The modus-operandi-owned files under the scripts dir (one key per installed
...
# cleaned up so `modus-operandi uninstall` fully clears a machine installed before
...
def pip_installed_bin(paths: Paths) -> set[Path]:
    ...
    owned: set[Path] = set()
    try:
        dist = importlib.metadata.distribution("modus-operandi")
...
def do_uninstall(paths: Paths, yes: bool) -> int:
    """Remove every modus-operandi-owned file; returns the exit code.

    Deleting ``~/.config/modus-operandi`` removes the user's config.yml, so the
    ...
    if pip_owned:
        print(
            f"kept the pip console scripts in {paths['user_bin']} (owned by the installed package)"
        )
    print(
        "run `pip uninstall modus-operandi` to remove the installed package "
