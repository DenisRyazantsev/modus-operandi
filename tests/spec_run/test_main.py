...
from spec_run import InstallError, __version__, bootstrap, installer, verify
...
class MainTest(unittest.TestCase):
    ...
    def test_main_bootstraps_before_exec(self):
    ...
    def test_main_help_exit_zero_without_bootstrap(self):
        mod = self.mod
    ...
    def test_main_dangling_input_exit_nonzero(self):
        ...
        for argv in (["task", "feat", "-i"], ["review", "-i"]):
            with (
                self.subTest(argv=argv), mock.patch("sys.stderr", io.StringIO()) as err,
                mock.patch.object(mod.bootstrap, "ensure_installed") as ensure,
            ):
                rc = mod.main(argv)
            self.assertEqual(rc, 1)
    ...
    def test_main_edit_bootstraps_then_runs_edit(self):
        mod = self.mod
        with (
            mock.patch.object(mod.bootstrap, "ensure_installed", return_value=0),
    ...
    def test_main_edit_bootstrap_failure_aborts(self):
        mod = self.mod
        with (
            mock.patch.object(mod.bootstrap, "ensure_installed", return_value=1),
    ...
    def test_main_edit_propagates_edit_exit_code(self):
        mod = self.mod
        with (
            mock.patch.object(mod.bootstrap, "ensure_installed", return_value=0),
    ...
    def test_main_uninstall_skips_bootstrap_and_passes_yes(self):
        mod = self.mod
        with (
            mock.patch.object(mod.bootstrap, "ensure_installed") as ensure,
    ...
    def test_main_uninstall_without_yes(self):
        mod = self.mod
        with (
            mock.patch.object(mod.bootstrap, "ensure_installed") as ensure,
...
class EnsureInstalledTest(unittest.TestCase):
    ...
    def test_fresh_install_renders_and_prints_one_status_line(self):
        mod = self.mod
        captured = io.StringIO()
        with (
        ...
        self.assertEqual(
            ...
            f"spec-run: installed to {mod._LAYOUT['config_dir']}",
        )

    def test_stale_version_renders_and_prints_updated(self):
        mod = self.mod
        ...
        with (
            ...
            rc = bootstrap.ensure_installed(mod._LAYOUT)
        self.assertEqual(rc, 0)
        self.assertEqual(captured.getvalue().strip(), f"spec-run: updated to {__version__}")

    def test_current_install_prints_nothing(self):
        mod = self.mod
        self.marker.parent.mkdir(parents=True, exist_ok=True)
    ...
    def test_prerequisite_failure_reports_error(self):
        mod = self.mod
        with (
            mock.patch.object(
    ...
    def test_apply_failure_reports_error(self):
        mod = self.mod
        with (
            mock.patch.object(verify, "check_prerequisites"),
