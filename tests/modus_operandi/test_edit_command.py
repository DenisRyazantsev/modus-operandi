"""Unit tests for edit_command._run_edit (the `modus-operandi edit` flow)."""
...
from modus_operandi import InstallError, edit_command, installer, paths
...
class EditCommandTest(unittest.TestCase):
    ...
    def _patch_editor_chain(
        self, editor: str = "/usr/bin/nano"
    ) -> tuple[mock._patch[Any], mock._patch[Any], mock._patch[Any]]:
        return (
    ...
    def test_edit_launches_editor_then_applies(self) -> None:
        ...
        def fake_run(cmd: list[str], check: bool = True) -> mock.Mock:
            calls.append(cmd)
    ...
    def test_edit_applies_after_nonzero_editor_exit(self) -> None:
        ...
        with (
            mock.patch.object(edit_command, "resolve_editor", return_value=["nano"]),
            mock.patch.object(edit_command.subprocess, "run", return_value=mock.Mock(returncode=7)),
            mock.patch.object(installer, "apply", return_value=(False, {})) as apply,
            mock.patch("sys.stdout", mock.Mock()),
        ):
            rc = edit_command._run_edit(self.config, self.layout)
    ...
    def test_edit_invalid_config_fails_with_error(self) -> None:
        ...
        with (
            mock.patch.object(edit_command, "resolve_editor", return_value=["nano"]),
            mock.patch.object(edit_command.subprocess, "run", return_value=mock.Mock(returncode=0)),
            mock.patch.object(installer, "apply", side_effect=InstallError("invalid config.yml")),
