...
class UsageParserTest(unittest.TestCase):
    ...
    def test_result_usage_wins_over_flat_fields_of_the_same_event(self):
    ...
    def test_nested_cache_overrides_flat_cache_names_per_field(self):
        ...
        self.assertEqual(
            parser.event_usage(
                {"type": "result", "usage": {"cache": {"read": 7}}}
            )["cache_read"],
            7,
        )

    def test_no_usage_key_consults_flat_and_step_finish_branches(self):
        parser = self._parser()
        self.assertEqual(
            parser.event_usage({"type": "result", "inputTokens": 100})["input"], 100
        )
        self.assertEqual(parser.event_usage(_step_finish(input=100))["input"], 100)

    def test_prefer_result_disables_step_finish_fallback(self):
        parser = self._parser()
        ...
        self.assertEqual(
            parser.event_usage({"type": "result", "usage": {"inputTokens": 5}}, True)[
                "input"
            ],
            5,
        )
    ...
    def test_is_result_style_detects_the_modern_shapes(self):
        parser = self._parser()
        self.assertTrue(
            parser.is_result_style({"type": "result", "usage": {"inputTokens": 1}})
        )
    ...
    def test_unrecognized_usage_shape_parses_to_none(self):
