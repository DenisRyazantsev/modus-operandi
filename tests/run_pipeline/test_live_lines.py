...
class LiveLinesTest(unittest.TestCase):
    ...
    def test_fix_all_fixes_and_reopens_the_block(self):
        mod = load_run_pipeline()
        live = mod.LiveLines(tty=True)
        live.set_step("study", 3)
        live.add_event("planner", "", _step_finish())
    ...
    def test_any_event_activates_the_process(self):
    ...
    def test_non_dict_events_are_skipped_not_crash(self):
    ...
    def test_non_tty_event_without_usage_prints_nothing(self):
        # On a non-TTY only events WITH usage print a line: one plain line
        # per agent turn (ADR-0012).
        mod = load_run_pipeline()
        live = mod.LiveLines(tty=False)
        self.assertIsNone(
            live.add_event("planner", "", {"type": "text", "part": {"type": "text"}})
        )
        line = live.add_event("planner", "", _step_finish())
        self.assertIsNotNone(line)

    def test_non_tty_cursor_usage_event_prints_one_line(self):
        mod = load_run_pipeline()
    ...
    def test_non_tty_reopens_line_with_current_step_label_after_fix(self):
...
class SpinnerTest(unittest.TestCase):
    ...
    def test_harness_line_contains_spinner_between_label_and_step(self):
        mod = load_run_pipeline()
        live = mod.LiveLines(total_steps=15, tty=True)
        live.set_step("study", 3)
        line = live.block()[0]
    ...
    def test_process_line_spinner_between_label_and_step(self):
        mod = load_run_pipeline()
        live = mod.LiveLines(total_steps=15, tty=True)
        live.set_step("study", 3)
        live.add_event("planner", "", _step_finish())
...
class HarnessLineTest(unittest.TestCase):
    ...
    def test_harness_line_before_any_event(self):
        mod = load_run_pipeline()
        live = mod.LiveLines(total_steps=15, tty=True)
        live.set_step("study", 3)
        with mock.patch.object(
    ...
    def test_harness_line_without_step_omitted(self):
        # No known step yet (run id not discovered): nothing to draw.
        mod = load_run_pipeline()
        live = mod.LiveLines(tty=True)
        self.assertEqual(live.block(), [])

    def test_first_event_replaces_harness_line(self):
        mod = load_run_pipeline()
        live = mod.LiveLines(total_steps=15, tty=True)
        live.set_step("study", 3)
    ...
    def test_parallel_forks_show_one_line_per_fork(self):
        mod = load_run_pipeline()
        live = mod.LiveLines(tty=True)
        live.set_step("review-fan", 10)
    ...
    def test_fix_all_fixes_harness_line_too(self):
    ...
    def test_fixed_harness_keeps_completed_step_label(self):
    ...
    def test_fixed_process_line_keeps_completed_step_label(self):
    ...
    def test_harness_belongs_to_another_step_is_left_live(self):
    ...
    def test_fix_all_fixes_process_lines(self):
        mod = load_run_pipeline()
        live = mod.LiveLines(total_steps=15, tty=True)
        live.set_step("study", 3)
        live.add_event("planner", "", _step_finish())
    ...
    def test_non_tty_step_line_once_per_step_change(self):
        mod = load_run_pipeline()
        live = mod.LiveLines(total_steps=15, tty=False)
        with mock.patch.object(
            sys.modules["live_lines"], "stamp", return_value="07:51:37"
    ...
    def test_set_step_reports_only_non_empty_changes(self):
        mod = load_run_pipeline()
        live = mod.LiveLines(tty=False)
...
class CursorUsageTest(unittest.TestCase):
    ...
    def test_result_usage_camel_case_with_nested_cache(self):
        mod = load_run_pipeline()
    ...
    def test_result_usage_flat_cache_tokens(self):
        mod = load_run_pipeline()
        live = mod.LiveLines(tty=True, backend="cursor")
        live.add_event(
            "planner",
    ...
    def test_top_level_token_fields_of_the_event(self):
        mod = load_run_pipeline()
        live = mod.LiveLines(tty=True, backend="cursor")
        live.add_event(
            "planner", "", {"type": "result", "inputTokens": 10, "outputTokens": 20}
        )
        self.assertIn("input 10", live.block()[0])
    ...
    def test_cursor_accumulates_across_events(self):
        mod = load_run_pipeline()
        live = mod.LiveLines(tty=True, backend="cursor")
        live.add_event(
            "planner",
    ...
    def test_step_finish_fallback_on_cursor_run(self):
    ...
    def test_step_finish_fallback_ignored_after_result_usage(self):
    ...
    def test_step_finish_fallback_before_any_result_event_still_counts(self):
    ...
    def test_malformed_result_event_does_not_disable_step_finish_fallback(self):
    ...
    def test_unrecognized_usage_shape_does_not_disable_step_finish_fallback(self):
    ...
    def test_price_shown_only_on_opencode(self):
        mod = load_run_pipeline()
    ...
    def test_malformed_cursor_usage_skipped_whole(self):
        mod = load_run_pipeline()
        live = mod.LiveLines(tty=True, backend="cursor")
        malformed = [
        ...
        # A valid event afterwards still accumulates.
        live.add_event(
            "planner", "", {"type": "result", "usage": {"inputTokens": 10}}
        )
        self.assertIn("input 10", live.block()[0])

    def test_event_without_usage_keeps_line_tokenless(self):
