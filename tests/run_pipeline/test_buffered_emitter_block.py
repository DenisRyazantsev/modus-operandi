...
class BufferedEmitterBlockTest(unittest.TestCase):
    ...
    def _emitter(self, mod):
        gate = mod.GateState()
        emitter = mod.BufferedEmitter(gate)
        return gate, emitter

    def _capture(self, emitter, fn):
        captured = io.StringIO()
        with mock.patch("sys.stdout", captured):
            fn()
        return captured.getvalue()

    def test_redraw_stays_in_place(self):
    ...
    def test_shorter_block_erases_leftover_rows(self):
        mod = load_run_pipeline()
        _, emitter = self._emitter(mod)
    ...
    def test_normal_output_keeps_block_visible_below(self):
        mod = load_run_pipeline()
        _, emitter = self._emitter(mod)
    ...
    def test_fixed_lines_replace_block_no_stale_redraw(self):
        mod = load_run_pipeline()
        _, emitter = self._emitter(mod)
        self._capture(emitter, lambda: emitter.emit_live(["a"]))
        out = self._capture(
            ...
            lambda: (
                ...
                emitter.emit_step(
                    "write-adr", {"status": "completed", "output": {"stdout": ""}}
                ),
            ),
        )
    ...
    def test_clear_live_leaves_cursor_at_block_start(self):
        mod = load_run_pipeline()
        _, emitter = self._emitter(mod)
    ...
    def test_block_lines_are_clamped_to_terminal_width(self):
        ...
        with mock.patch(
            ...
            out = self._capture(
                emitter, lambda: emitter.emit_live(["x" * 50])
            )
