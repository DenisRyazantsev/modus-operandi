...
class WorkflowStepIdsTest(unittest.TestCase):
    ...
    def test_returns_top_level_step_ids_from_source_path(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(
                mod.workflow_step_ids(str(path)), ["a", "b", "c"]
            )
    ...
    def test_resolves_bare_id_from_installed_config_dir(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch.object(
                mod._run_pipeline_common, "CONFIG_DIR", config_dir
            ):
                self.assertEqual(
                    mod.workflow_step_ids("task-pipeline"), ["a", "b"]
                )
    ...
    def test_unparseable_yaml_degrades_to_none(self):
        mod = load_run_pipeline()
        with tempfile.TemporaryDirectory() as tmp:
            path = Path(tmp) / "wf.yml"
    ...
    def test_wrong_document_shape_degrades_to_none(self):
        mod = load_run_pipeline()
        with tempfile.TemporaryDirectory() as tmp:
            path = Path(tmp) / "wf.yml"
...
class StepMarkerIndexTest(unittest.TestCase):
    ...
    def test_finds_the_step_position(self):
        mod = load_run_pipeline()
    ...
    def test_loop_iteration_suffix_uses_the_parent_step(self):
    ...
    def test_unknown_step_degrades_to_none(self):
        mod = load_run_pipeline()
        ids = ["study", "research"]
        self.assertIsNone(mod.step_marker_index("no-such-step", ids))

    def test_no_workflow_ids_degrades_to_none(self):
        mod = load_run_pipeline()
...
class StepMarkerTest(unittest.TestCase):
    ...
    def test_step_marker_includes_nm(self):
        mod = load_run_pipeline()
        captured = io.StringIO()
        with (
            mock.patch("sys.stdout", captured),
            mock.patch.object(
                mod._run_pipeline_common, "stamp", return_value="07:51:37"
            ),
        ):
            mod.print_step_result(
                "study", {"status": "completed", "output": {"stdout": ""}}, 3, 15
            )
        self.assertIn(
            "[07:51:37] --- step study (completed) [4/15]", captured.getvalue()
        )

    def test_step_marker_without_progress(self):
        mod = load_run_pipeline()
        captured = io.StringIO()
        with mock.patch("sys.stdout", captured):
            mod.print_step_result(
                "study", {"status": "completed", "output": {"stdout": ""}}
            )
