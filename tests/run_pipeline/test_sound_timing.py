...
class SoundTimingTest(unittest.TestCase):
    ...
    the same play_signal() call, never a different signal."""
    ...
    def _run_main(
        ...
        # forwarding thread): these tests are about play_signal() timing,
        # not the pty plumbing (covered by FeedbackGateFlowTest). The
        # signal calls live in the split modules (SRP): the gate-open
        # signal fires in stdout_reader.py and the finish signal in
        # run_finish.py, so the shared mock replaces the name in both
        # consumer namespaces.
        signal = mock.Mock()
        with (
            cwd(tmp),
    ...
    def test_success_fires_signal(self) -> None:
        mod = load_run_pipeline()
        with tempfile.TemporaryDirectory() as tmp:
            rc, play_signal = self._run_main(mod, tmp, ["Run ID: abc12345"], 0)
        self.assertEqual(rc, 0)
        self.assertEqual(play_signal.call_count, 1)

    def test_failure_fires_signal(self) -> None:
        mod = load_run_pipeline()
        with tempfile.TemporaryDirectory() as tmp:
            rc, play_signal = self._run_main(mod, tmp, ["Run ID: abc12345"], 1)
