...
_SPEC = importlib.util.spec_from_file_location(
    "check_questions", REPO_ROOT / "src/modus_operandi/data/pipeline_scripts" / "check_questions.py"
)
...
class CmdCheckQuestionsTest(unittest.TestCase):
    ...
    def test_none_first_line_exits_zero(self) -> None:
        (self.task_dir / "questions.md").write_text("QUESTIONS: NONE\n", encoding="utf-8")
        with self.assertRaises(SystemExit) as cm:
            self.check()
        self.assertEqual(cm.exception.code, 0)

    def test_present_first_line_exits_one(self) -> None:
    ...
    def test_empty_file_exits_one(self) -> None:
        (self.task_dir / "questions.md").write_text("", encoding="utf-8")
        with self.assertRaises(SystemExit) as cm:
            self.check()
        self.assertEqual(cm.exception.code, 1)

    def test_leading_whitespace_on_marker_is_tolerated(self) -> None:
        (self.task_dir / "questions.md").write_text("  QUESTIONS: NONE\n", encoding="utf-8")
        with self.assertRaises(SystemExit) as cm:
            self.check()
        self.assertEqual(cm.exception.code, 0)

    def test_marker_not_on_first_line_exits_one(self) -> None:
        (self.task_dir / "questions.md").write_text("1. what?\nQUESTIONS: NONE\n", encoding="utf-8")
        with self.assertRaises(SystemExit) as cm:
            self.check()
        self.assertEqual(cm.exception.code, 1)
