...
_SPEC = importlib.util.spec_from_file_location(
    "task_utils", REPO_ROOT / "src/modus_operandi/data/pipeline_scripts" / "task_utils.py"
)
...
class ResolveTaskDirTest(unittest.TestCase):
    ...
    def test_empty_task_id_resolves_symlink(self) -> None:
    ...
    def test_empty_task_id_without_symlink_fails(self) -> None:
        with self.assertRaises(SystemExit):
            task_utils.resolve_task_dir(str(self.root / ".workflow"), "")

    def test_empty_task_id_broken_symlink_fails(self) -> None:
        (self.tasks / "current").symlink_to("missing-task")
        with self.assertRaises(SystemExit):
            task_utils.resolve_task_dir(str(self.root / ".workflow"), "")

    def test_empty_task_id_with_real_dir_fails(self) -> None:
