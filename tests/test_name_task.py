...
class NameTaskTest(unittest.TestCase):
    ...
    def _run(self, feature: str, **env_extra) -> subprocess.CompletedProcess:
    ...
    def test_cursor_uses_text_output_and_returns_trimmed_slug(self):
        self._write_executable(self.bin / "cursor-agent", CURSOR_AGENT_SCRIPT)
        result = self._run("build a kanban board", SKLC_BACKEND="cursor")
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertEqual(result.stdout.strip(), "kanban-board")
        args = self._args(self.cursor_log)
        self.assertEqual(args[:6], ["-p", "--output-format", "text", "--force",
                                    "--trust", "--model"])
    ...
    def test_cursor_missing_model_errors(self):
        self._write_executable(self.bin / "cursor-agent", CURSOR_AGENT_SCRIPT)
        result = self._run(
            "build a kanban board", SKLC_BACKEND="cursor", SKLC_EXECUTOR_MODEL=""
        )
        self.assertEqual(result.returncode, 2)
        self.assertIn("SKLC_EXECUTOR_MODEL", result.stderr)

    def test_opencode_default_uses_json_stream(self):
        ...
        self.assertEqual(args[:5], ["run", "--agent", "executor", "--auto",
                                    "--format"])
