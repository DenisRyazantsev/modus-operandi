...
then nano, then vi) used by `spec-run edit` and as the feedback gates'
...
the spec-run launcher's `edit` command (edit_command.py) — and it must behave
identically for both. The installer copies this module next to both consumers
(scripts/ and the launcher's bin dir).
"""
